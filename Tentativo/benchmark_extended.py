...
Uso:
  python benchmark_extended.py --csv tracks_with_clusters.csv --T 200 --seeds 20 --out benchmark_results
...
def run_random(df: pd.DataFrame, sim: UserSimulator, T: int, seed: int) -> Tuple[np.ndarray, np.ndarray]:
...
def run_popularity(df: pd.DataFrame, sim: UserSimulator, T: int) -> Tuple[np.ndarray, np.ndarray]:
    unseen = df.copy()
...
# -----------------------
# Runner: TS gerarchico (Il nostro)
# -----------------------

def run_ts_hier(df: pd.DataFrame, sim: UserSimulator, feature_cols: List[str], T: int, seed: int) -> Tuple[np.ndarray, np.ndarray]:
...
def run_cluster_ts_hybrid(df: pd.DataFrame, sim: UserSimulator, T: int, seed: int, cfg: Optional[HybridConfig] = None) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def aggregate_and_plot(all_rewards: Dict[str, List[np.ndarray]], all_ptrues: Dict[str, List[np.ndarray]], T: int, out_dir: str) -> None:
    ...
    plt.title("Benchmark: cumulative like rate")
    plt.legend()
    plt.tight_layout()
    ...
    plt.title("Benchmark: rolling like rate")
    plt.legend()
    plt.tight_layout()
    ...
    plt.title("Benchmark: expected like rate (simulator ground truth)")
    plt.legend()
    plt.tight_layout()
...
def main() -> None:
    ...
    ap.add_argument("--run_name", default="", help="Nome cartella run (opzionale)")
