...
def make_unique_run_dir(base_out: str, *, sim: str, T: int, seeds: int, run_name: Optional[str] = None) -> str:
    """Create a unique run subdirectory to avoid overwriting outputs.

    If run_name is provided, it will be used as the subdirectory name; otherwise a timestamp-based
    name is generated. If the directory already exists, a random suffix is appended.
    """
    ...
    return out_dir
GenreOnlyUserSimulator, GenreOnlyUserConfig
...
def main() -> None:
    ...
    ap.add_argument("--seeds", type=int, default=1, help="Numero seed (ripetizioni)")
    ...
    ap.add_argument("--noise", type=float, default=0.2, help="Rumore del simulatore utente")
    ...
    out_dir = make_unique_run_dir(args.out, sim=str(args.sim), T=int(args.T), seeds=int(args.seeds), run_name=getattr(args, 'run_name', None))
